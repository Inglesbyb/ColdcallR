  ...
  while (hasMore) {
    try {
      ...
      const recordsToUpsert = validItems.map((item: any) => {
        ...
        // Assign risk tag based on SIC code (same logic as before)
        let riskTag = "DEFAULT";
...
async function run() {
  console.log("Starting Liverpool Territory Ingestion...");
  let totalOverall = 0;

  for (const prefix of LIVERPOOL_POSTCODES) {
    const totalForPrefix = await ingestPostcode(prefix);
    totalOverall += totalForPrefix;
    console.log(`[PROGRESS] Completed ${prefix}. Total businesses ingested so far: ${totalOverall}`);
  }

  console.log(`\n=== INGESTION COMPLETE ===`);
  console.log(`Total active businesses upserted: ${totalOverall}`);
  process.exit(0);
}
