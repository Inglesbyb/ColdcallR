...
export default function LeafletMap({ filter }: LeafletMapProps) {
  ...
  return (
    <div className="relative w-full h-full">
      ...
      {loading && (
        <div className="absolute top-4 left-1/2 -translate-x-1/2 z-[1001] flex items-center gap-2 px-3 py-1.5 rounded-full bg-slate-900/90 backdrop-blur border border-slate-700/60 shadow-lg">
      ...
      {!loading && filteredLeads.length > 0 && (
        <div className="absolute top-4 left-1/2 -translate-x-1/2 z-[1001] px-3 py-1.5 rounded-full bg-slate-900/90 backdrop-blur border border-slate-700/60 shadow-lg">
      ...
      {!loading && filteredLeads.length === 0 && (
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-[1001] max-w-xs text-center">
          <div className="bg-slate-900/95 backdrop-blur rounded-2xl border border-slate-700 p-6 shadow-xl">
            <p className="text-2xl mb-2">🗺️</p>
            <p className="text-white font-semibold mb-1">No leads yet</p>
            <p className="text-sm text-slate-400">
              Run <code className="text-blue-400">POST /api/seed</code> to pull
